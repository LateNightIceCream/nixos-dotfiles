...
let
  hyprratio = { bc }:
    pkgs.writeShellScriptBin "hyprratio" ''
      if [ $# -ne 1 ]; then
  ...
  gresize = { bc, jq }:
    pkgs.writeShellScriptBin "gresize" ''
...
in {
  ...
  imports = [ (import ../../environment/hyprland-environment-variables.nix) ];
  ...
  home.file.".config/hypr/scripts/dropdown.sh".source =
    ./config/additional/scripts/dropdown.sh;
